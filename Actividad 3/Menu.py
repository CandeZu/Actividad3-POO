...
class Menu:
    ...
    def opcion1(self, cam, cos):
        ...
        band = False
        while not band:
            camion = cam.buscarCamion(ValidaEntero('Ingrese ID de camión: '))
            if camion != None:
                cos.kilos(camion.getIdentificador(), camion.getTara())
                band = True
            else:
    ...
    def opcion2(self, cam, cos):
        ...
        while not band:
            ...
            if ( dia >= 1 and dia <= 45 ):
                ...
                for i in range(20):
                    camion = cam.buscarCamion(i + 1)
                    print('%10s' % (camion.getPat()), '%13s' % (camion.getCond()), '%10s' % (cosecha.getValor((camion.getId()) - 1, dia - 1)))
                band = True
            else:
